...
def _(datetime, df, pl):
    cols = ["plan_start_dt", "plan_stop_departure_dt", "start_dt", "gtfs_travel_to_dt", "gtfs_arrival_dt", "tm_scheduled_time_dt", "tm_actual_arrival_dt", "tm_actual_departure_dt"]
    ...
    check_all_times_in_timezone = df.drop_nulls(cols).with_columns(
        [
            (pl.col("plan_start_dt").dt.replace_time_zone(None) - pl.col(c).dt.replace_time_zone(None)).alias(f"{c}_diff")
            for c in cols
