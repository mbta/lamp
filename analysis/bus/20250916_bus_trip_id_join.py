...
def _(gtfs_event, pl):
    ...
    unders = unders.with_columns(
        pl.col("trip_id").alias("trip_id_gtfs"), pl.col("trip_id").str.replace(r"_[1,2]", "")
...
def _(gtfs_event, pl):
    g1 = gtfs_event.with_columns(
        pl.col("trip_id").alias("trip_id_gtfs"), pl.col("trip_id").str.replace(r"-OL+", "")
    ).with_columns(
        pl.col("trip_id").str.replace(r"_[1,2]", "").alias("trip_id"),
    )
...
def _(g1, pl):
    (
        g1.filter(pl.col("trip_id").str.contains("OL")).is_empty()
        & g1.filter(pl.col("trip_id").str.contains("_1")).is_empty()
        & g1.filter(pl.col("trip_id").str.contains("_2")).is_empty()
    )
    return
