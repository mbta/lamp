...
from lamp_py.flashback.events import StopEventsJSON, StopEventsWithStatus, StopEventsTable, unnest_vehicle_positions, unnest_vehicle_positions_new
...
async def get_vehicle_positions_new(
    url: str = "https://cdn.mbta.com/realtime/VehiclePositions_enhanced.json",
    sleep_interval: int = 3,
    max_retries: int = 10,
) -> tuple[dy.DataFrame[StopEventsWithStatus], dy.DataFrame[VehiclePositions]]:
    ...
    return unnest_vehicle_positions_new(valid), unnest_vehicle_positions(valid)
...
def write_stop_events(stop_events: dy.DataFrame[StopEventsJSON], location: S3Location = stop_events_location, local_override: str | None = None) -> None:
    """Write stop events to specified location."""
    output_path = local_override or location.s3_uri
    process_logger = ProcessLogger("write_stop_events", s3_uri=output_path)
    process_logger.log_start()
    stop_events.write_json(output_path)
    process_logger.log_complete()
