...
To access the s3 buckets that hold LAMP data, you need to authenticate using their IAM account, which DuckDB supports natively via the DuckDB `aws` extension.
...
I have already [authenticated using `awscli`](https://docs.aws.amazon.com/cli/latest/userguide/cli-configure-files.html#cli-configure-files-methods) so I can tell DuckDB to use those credentials.
...
Since DuckDB doesn't persist credentials between sessions, you'll need to **run these next lines each time you start a DuckDB session**.
...
Once authenticated, attach the Lightswitch data catalog.
This is a DuckDB database that only holds views of LAMP Parquet URIs.
For instance, the view for `RT_VEHICLE_POSITIONS` contains logic that lists the URLs for each `RT_VEHICLE_POSITIONS` file in LAMP's springboard bucket.
