...
class TMDailyWorkPiece(dy.Schema):
    ...
    logon_time = dy.Datetime(nullable=True, time_zone=None)  # seconds after midnight converted to UTC..then None
...
def create_public_operator_id_map(daily_work: pl.DataFrame, seed: float = datetime.now().timestamp()) -> pl.DataFrame:
    """
    Create a daily unique mapping between operator badge and a randomized
    public identifier to tie together runs/trips and operators
    ...
    operator_id_mapping = (
        ...
        .unique(subset="tm_trip_id", maintain_order=True)
        .with_columns(
            pl.Series(random.sample(range(10000, 99999), daily_work["tm_trip_id"].unique(maintain_order=True).len()))
            .cast(pl.String)
...
def get_daily_work_pieces(daily_work_piece_files: List[str]) -> dy.DataFrame[TMDailyWorkPiece]:
    """
    ...
    public_operator_mapping = create_public_operator_id_map(operator_work_pieces, datetime.now().timestamp())
    ...
    public_daily_work_piece = process_logger.log_dataframely_filter_results(
        *TMDailyWorkPiece.filter(daily_with_public_operator_id)
    )

    process_logger.log_complete()

    return public_daily_work_piece
